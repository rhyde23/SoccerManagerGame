...
from do_best_lineup import best_lineup
from morale_losses import get_values
from morale_gains import get_values_gains
#from change_development_based_on_morale import get_new_change
...
def game(save_number) :
    ...
    last_days_of_regular_season = {
        ...
        'English League One':'May 9',
        'English League Two':'May 9',
    }
    
    days_of_finals = {
        'English Championship':'May 29',
        'English League One':'May 30',
        'English League Two':'May 31',
    }
    
    day_of_season_reset = 'June 1'
    ...
    """
    reward_for_win, reward_for_draw = 40, 16
    def get_morale_reward(team_names, team_name) :
        team_total_ratings = {tn:sum([int(current_data[tn+'_Players'][tn_pname]['Rating']) for tn_pname in current_data[tn+'_Lineup']]) for tn in team_names}
        ttr_sorted = sorted(list(team_total_ratings.keys()), key=lambda x: team_total_ratings[x], reverse=True)
        percentage = (ttr_sorted.index(team_name))/len(ttr_sorted)
        return (reward_for_win)+(reward_for_win*percentage), (reward_for_draw)+(reward_for_draw*percentage)
    """
    ...
    days_values_gains = get_values_gains()
    ...
    def handle_playoff_game(winner, team1, team2, final_team1_score, final_team2_score) :
        ...
        if current_data['WithoutYear'] != playoff_final_date :
            ...
            if current_data['WithoutYear'] in current_data['SpecialGamesResults'] :
                current_data['SpecialGamesResults'][current_data['WithoutYear']].append([winner, final_team1_score, final_team2_score])
            else :
                current_data['SpecialGamesResults'][current_data['WithoutYear']] = [[winner, final_team1_score, final_team2_score]]
    ...
    def sim_other_games(games_in_current_month) :
        if current_data['WithoutYear'] in games_in_current_month :
            games_in_day = games_in_current_month[current_data['WithoutYear']]
            for other_game in games_in_day :
                try :
                    team1, team2 = other_game
                except :
                    print(current_data['WithoutYear'], other_game)
                    quit()
                ...
                if team1 != team and team2 != team :
                    ...
                    team1_rating = calculate_rating(team1_players, team1_lineup, team1_formation, energy_significance_percentage, {p_key:team1_players[p_key]['Energy'] for p_key in team1_players})
                    team2_rating = calculate_rating(team2_players, team2_lineup, team2_formation, energy_significance_percentage, {p_key:team2_players[p_key]['Energy'] for p_key in team2_players})
                    while True :
                        can_break = True
                        ...
                        if current_data['WithoutYear'] in current_data['SpecialGames'] :
    ...
    calendar_buttons = [
        [250, 120, 100, 30, white, light_blue, myfont3.render("START SIM", True, light_blue), myfont3.render("START SIM", True, white)],
        [250, 120, 100, 30, light_blue, white, myfont3.render("END SIM", True, white), myfont3.render("END SIM", True, light_blue)],
        [250, 120, 100, 30, white, light_blue, myfont3.render("PLAY MATCH", True, light_blue), myfont3.render("PLAY MATCH", True, white)],
        [400, 120, 120, 30, white, light_blue, myfont3.render("SAVE PROGRESS", True, light_blue), myfont3.render("SAVE PROGRESS", True, white)]
    ]
    ...
    if current_data['StoppedOnMatchday'] :
    ...
    over_save_progress_button = False
    ...
    def display_players(player_name, player_rating, rating_coords, coords, position, p_position, morale , ratings_lost=0) :
    ...
    def display_starting_lineup() :
        ...
        for position_index, position in enumerate(positions_in_order) :
            ...
            dudes_morale = current_data[team+'_Players'][player_name]['Morale']
            display_players(player_name, player_rating, rating_coords, coords, position, player_position, dudes_morale, ratings_lost=int(current_data[team+'_Players'][player_name]['Positions'][convert_outside_position_to_center(current_data[team+'_Players'][player_name]['Position'])]) - int(current_data[team+'_Players'][player_name]['Positions'][convert_outside_position_to_center(position)]))
    ...
    def display_bench_players() :
        for bench_index in range(len(current_bench_page_players)) :
            ...
            rating_coords = get_correct_rating_coordinates(coords)
            dudes_morale = current_data[team+'_Players'][player_name]['Morale']
            display_players(player_name, player_rating, rating_coords, coords, 'SUB', player_position, dudes_morale)
    
    
    energy_significance_percentage = 30
    

    current_data['CurrentTeamRating'] = calculate_rating(current_data[team+'_Players'], current_data['CurrentLineup'], current_data['CurrentFormation'], energy_significance_percentage, {p_key:100 for p_key in current_data['CurrentLineup']})
    ...
    def new_formation_lefts_and_rights(positions_in_order, lineup_in_order, team_key_name) :
        real_positions = {n:current_data[team_key_name+'_Players'][n]['Position'] for n in lineup_in_order}
        for i in range(11) :
    ...
    space_key_texts = []
    for space_key_rgb in range(255, 120, -3) :
        space_key_texts.append(myfont.render('Hold the space key to stop sim.', True, (space_key_rgb, space_key_rgb, space_key_rgb)))
    space_key_texts_index = 0
    space_key_texts_index_direction = 1
    length_of_space_key_texts = len(space_key_texts)
    
    space_key_text_x = 250
    
    saving_text = myfont.render('Saving Game...', True, white)
    saving_text_x = space_key_text_x+int((space_key_texts[0].get_width()-saving_text.get_width())/2)
    
    #Morale/Development Settings
    
    in_lineup_expo_base = 1.11
    in_lineup_expo_x_adder = 18
    in_lineup_expo_y_adder = 15
    not_in_lineup_expo_base = 1.15
    not_in_lineup_expo_x_adder = 15
    
    harshest_day_projection = 125
    expected_morale = 90
    morale_can_help = False
    
    #Energy Settings
    
    max_energy_drop = 65
    daily_energy_gain = 5
    minimum_considered_stamina, maximum_considered_age, maximum_considered_stamina, minimum_considered_age = 45, 40, 99, 16
    unanimous_drop_percentage, stamina_drop_percentage, age_drop_percentage = 40, 40, 20
    
    er_deviation = 10

    def calculate_energy_deduction(stamina, age, max_considered_stamina, min_considered_stamina) :
        unanimous_deduction = max_energy_drop*(unanimous_drop_percentage/100)
        stamina_deduction = max_energy_drop*(stamina_drop_percentage/100)
        age_deduction = max_energy_drop*(age_drop_percentage/100)
            
        stamina_deduction = stamina_deduction*((max_considered_stamina-stamina)/(max_considered_stamina-min_considered_stamina))
        age_deduction = age_deduction*((age-minimum_considered_age)/(maximum_considered_age-minimum_considered_age))
        if stamina_deduction < 0 :
            stamina_deduction = 0
        if age_deduction < 0 :
            age_deduction = 0
        return int(sum([unanimous_deduction, stamina_deduction, age_deduction]))
        
    
    energy_same_positions = {
        'Goalkeeper':['GK'],
        'Outside Back':['LB', 'RB', 'LWB', 'RWB'],
        'Center Back':['LCB', 'CB', 'RCB'],
        'Midfielder':['LDM', 'CDM', 'RDM', 'LCM', 'CM', 'RCM', 'LAM', 'CAM', 'RAM'],
        'Winger':['LM', 'RM', 'LW', 'RW'],
        'Striker':['LS', 'ST', 'RS', 'CF']
    }
    
    def find_energy_deduction_category(pos) :
        for esp_key in energy_same_positions :
            if pos in energy_same_positions[esp_key] :
                return esp_key
    ...
    while manager_loop :
                ...
                for event in pygame.event.get() :
                    if event.type == pygame.MOUSEBUTTONDOWN :
                        if currently_hovering_formation :
                            ...
                            current_data['CurrentLineup'] = new_formation_lefts_and_rights(get_positions_from_formation(hovering_formation), rearrange_lineup_formation_change(hovering_formation, new_posi), team)
        ...
        while calendar :
            ...
            if match_simming :
                ...
                #Display the scoreboard. If the variable "current_data['NextLeftOrRight']" variable == 0, it means that the user is the home
                if current_data['NextLeftOrRight'] == 0 :
                ...
                for event in pygame.event.get() :
                    if event.type == pygame.MOUSEBUTTONDOWN :
                        ...
                        if over_button2 :
                            ...
                            #The "current_data['StoppedOnMatchday']" variable tells us whether or not the next simming action will be to play the match
                            current_data['StoppedOnMatchday'] = False
                ...
                if not current_data['StoppedOnMatchday'] :
                    correct_date, correct_next_month = advance_one_day(current_date)
                    correct_month, correct_day, correct_year = unpack_date(correct_date)
                    if correct_next_month :
                        correct_month_number = get_month_number(correct_month)
                        month = get_number_month(correct_month_number)
                        current_calendar_screen = calendar_screens[correct_month_number]
                        games_in_current_month, dates_of_games_in_month = update_game_month_stuff()
                        month = get_number_month(correct_month_number-1)
                else :
                    correct_day = day
                ...
                display.blit(mr_manager_team, (60, 10))
                display.blit(mr_manager_name, (60, 30))
                display.blit(red_cover, coords_of_days[str(correct_day)])
                x, y = pygame.mouse.get_pos()
                ...
                for event in pygame.event.get() :
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN :
                        if over_save_progress_button and (not simming) :
                            display.blit(saving_text, (saving_text_x, 120))
                            pygame.display.update()
                            save_progress()
                        if over_sim_button and calendar_button_index != 1 :
                            if current_data['StoppedOnMatchday'] :
                                match_simming = True
                                stadium_text = myfont.render(current_data['NextStadium'], True, white)
                                stadium_text_width = stadium_text.get_width()
                                stadium_text_x = int((display_width-stadium_text_width)/2)
                                user_logo, user_logo_text, user_logo_x, user_logo_text_x, user_logo_y, user_logo_text_y = get_coords_of_logo_and_text(team, current_data['NextLeftOrRight'])
                                if current_data['NextLeftOrRight'] == 0 :
                                    opponent_lor = 1
                                else :
                                    opponent_lor = 0
                                opponent_logo, opponent_logo_text, opponent_logo_x, opponent_logo_text_x, opponent_logo_y, opponent_logo_text_y = get_coords_of_logo_and_text(current_data['NextOpponent'], opponent_lor)
                                ...
                                opponent_formation = current_data[current_data['NextOpponent']+'_Formation']
                                opponent_players = current_data[current_data['NextOpponent']+'_Players']
                                opponent_lineup = current_data[current_data['NextOpponent']+'_Lineup']
                                
                                
                                
                                user_rating = calculate_rating(current_data[team+'_Players'], current_data['CurrentLineup'], current_data['CurrentFormation'], energy_significance_percentage, {p_key:user_players[p_key]['Energy'] for p_key in current_data['CurrentLineup']})
                                opponent_rating = calculate_rating(opponent_players, opponent_lineup, opponent_formation, energy_significance_percentage, {p_key:opponent_players[p_key]['Energy'] for p_key in opponent_players})
                                print(user_rating, opponent_rating)
                                while True :
                                    can_break = True
                                    if current_data['NextLeftOrRight'] == 0 :
                                        winner, score_difference = match_sim(user_rating, opponent_rating, team, current_data['NextOpponent'])
                                        score, user_scorers, opponent_scorers = randomize_goals(user_players, opponent_players, user_lineup, opponent_lineup, team, current_data['NextOpponent'], user_formation, opponent_formation, winner, score_difference)
                                        s_one, s_two = score.split('-')
                                        final_user_score, final_opponent_score = int(s_one), int(s_two)
                                        user_scorers_text_x, opponent_scorers_text_x = 75, half+20
                                        update_standings_info(team, current_data['NextOpponent'], winner, final_user_score, final_opponent_score, standings_data_string, standingsinorder_data_string, standingsdata_data_string, topscorers_data_string, topscorersinorder_data_string, topassistors_data_string, topassistorsinorder_data_string, teamsinleague_data_string)
                                    
                                        if current_data['WithoutYear'] in current_data['SpecialGames'] :
                                            can_break = handle_playoff_game(winner, team, current_data['NextOpponent'], final_user_score, final_opponent_score)
                                    else :
                                        winner, score_difference = match_sim(opponent_rating, user_rating, current_data['NextOpponent'], team)
                                        score, opponent_scorers, user_scorers = randomize_goals(opponent_players, user_players, opponent_lineup, user_lineup, current_data['NextOpponent'], team, opponent_formation, user_formation, winner, score_difference)
                                        s_one, s_two = score.split('-')
                                        final_opponent_score, final_user_score = int(s_one), int(s_two)
                                        opponent_scorers_text_x, user_scorers_text_x = 75, half+20
                                        update_standings_info(current_data['NextOpponent'], team, winner, final_opponent_score, final_user_score, standings_data_string, standingsinorder_data_string, standingsdata_data_string, topscorers_data_string, topscorersinorder_data_string, topassistors_data_string, topassistorsinorder_data_string, teamsinleague_data_string)
                                    
                                        if current_data['WithoutYear'] in current_data['SpecialGames'] :
                                            can_break = handle_playoff_game(winner, current_data['NextOpponent'], team, final_opponent_score, final_user_score)
                                ...
                                currently_simming = False
                                if not current_data['WithoutYear'] in current_data['SpecialGames'] :
                                    ...
                                    for nop in opponent_scorers :
                                        current_data[current_data['NextOpponent']+'_Players'][nop]['Goals'] += 1
                                        if nop in current_data[topscorersinorder_data_string] :
                                            current_data[topscorers_data_string][nop] += 1
                                    
                                    if winner == 'Draw' :
                                        current_data[standings_data_string][team] += 1
                                        current_data[standings_data_string][current_data['NextOpponent']] += 1
                                    else :
                                        current_data[standings_data_string][winner] += 3
                                    
                                    put_standings_in_correct_order(standings_data_string, standingsinorder_data_string, standingsdata_data_string, topscorers_data_string, topscorersinorder_data_string, topassistors_data_string, topassistorsinorder_data_string, teamsinleague_data_string)
                                    update_top_scorers(user_scorers, team, opponent_scorers, current_data['NextOpponent'])
                                
                                
                                #Need to replicate for AI 
                                #morale_w, morale_d = get_morale_reward(current_data['TeamsInEachLeague'][current_data['CurrentLeague']], team)
                                gain_multi, loss_multi = 1, 0.75
                                
                                for user_player_not_playing in user_players :
                                    if user_player_not_playing in user_lineup :
                                        
                                        change = round(gain_multi*(in_lineup_expo_base**(calculate_rating(current_data[team+'_Players'], current_data[team+'_Lineup'], current_data[team+'_Formation'], energy_significance_percentage, {p_key:100 for p_key in current_data[team+'_Lineup']})-int(current_data[team+'_Players'][user_player_not_playing]['Rating'])+in_lineup_expo_x_adder)+in_lineup_expo_y_adder), 2)
                                        if change > 50.00 :
                                            change = 50.00
                                            
                                            
                                        
                                        current_data[team+'_Players'][user_player_not_playing]['Morale'] += change
                                        
                                        energy_avg = current_data[current_data['CurrentLeague']+'_EDeductions'][find_energy_deduction_category(current_data[team+'_Players'][user_player_not_playing]['Position'])]
                                        
                                        current_data[team+'_Players'][user_player_not_playing]['Energy'] -= calculate_energy_deduction(current_data[team+'_Players'][user_player_not_playing]['Player Stats']['Stamina'], int(current_data[team+'_Players'][user_player_not_playing]['Age']), energy_avg+er_deviation, energy_avg-er_deviation)
                                    else :
                                        change = round(loss_multi*(not_in_lineup_expo_base**(int(current_data[team+'_Players'][user_player_not_playing]['Rating'])-calculate_rating(current_data[team+'_Players'], current_data[team+'_Lineup'], current_data[team+'_Formation'], energy_significance_percentage, {p_key:100 for p_key in current_data[team+'_Lineup']})+not_in_lineup_expo_x_adder)), 2)
                                        if change > 25.00 :
                                            change = 25.00
                                        current_data[team+'_Players'][user_player_not_playing]['Morale'] -= change
                                        
                                        
                                    #Morale change based on team performance
                                    
                                    ########################################
                                    
                                    
                                    current_data[team+'_Players'][user_player_not_playing]['Morale'] = round(current_data[team+'_Players'][user_player_not_playing]['Morale'], 3)
                                    if current_data[team+'_Players'][user_player_not_playing]['Morale'] > 100.0 :
                                        current_data[team+'_Players'][user_player_not_playing]['Morale'] = 100.0
                                    elif current_data[team+'_Players'][user_player_not_playing]['Morale'] < 0 :
                                        current_data[team+'_Players'][user_player_not_playing]['Morale'] = 0
                                    else :
                                        pass
                                    #print(user_player_not_playing, '  Morale:', current_data[team+'_Players'][user_player_not_playing]['Morale'], '  Energy:', current_data[team+'_Players'][user_player_not_playing]['Energy'])
                                #print()
                                ...
                                if not simming :
                                    simming = True
                                    calendar_button_index += 1
                if calendar_button_index != 1 :
                    over_sim_button = display_button(calendar_buttons[calendar_button_index], (x, y))
                    over_save_progress_button = display_button(calendar_buttons[3], (x, y))
                else :
                    #display.blit(space_key_texts[space_key_texts_index], (250, 120))
                    pass
                if simming :
                    current_date, next_month = advance_one_day(current_date)
                    current_data['CurrentDate'] = current_date
                    month, day, year = unpack_date(current_date)
                    current_data['WithoutYear'] = ' '.join([month, str(day)])
                    done_count = 0
                    for league_progressing in leagues_in_game :
                        for team_progressing in current_data['TeamsInEachLeague'][league_progressing] :
                            ...
                            for player_progressing in current_data[key_progressing] :
                                player_stuff = current_data[key_progressing][player_progressing]
                                if current_data['WithoutYear'] == player_stuff['Date of Birth'] :
                                    player_stuff['Age'] = str(int(player_stuff['Age'])+1)
                                player_stuff['Energy'] += daily_energy_gain
                                if player_stuff['Energy'] > 100 :
                                    player_stuff['Energy'] = 100
                                player_stuff['Morales'].append(player_stuff['Morale'])
                                player_stuff['DaysUntilNextDevUpdate'] += 1
                                if player_stuff['DaysUntilNextDevUpdate'] == 20 :
                                    
                                    if player_stuff['DevelopmentPlanCountedDaysArray'+player_stuff['DevelopmentPlanNumber']] == [] :
                                        player_stuff['DevelopmentPlanNumber'] = str(int(player_stuff['DevelopmentPlanNumber'])+1)
                                    
                                    development_extension = player_stuff['DevelopmentPlanNumber']
                                        
                                    cont = False
                                    
                                    apply_to_first, apply_to_second = 0, 19
                                    
                                    player_stuff['DevelopmentPlanCountedDays'+development_extension] += 19
                                    if player_stuff['DevelopmentPlanCountedDays'+development_extension] >= player_stuff['DevelopmentPlanCountedDaysArray'+development_extension][player_stuff['DevelopmentPlanIndex'+development_extension]] :
                                        player_stuff['DevelopmentPlanCountedDays'+development_extension] = 0
                                        apply_to_first = player_stuff['DevelopmentPlanCountedDays'+development_extension]-player_stuff['DevelopmentPlanCountedDaysArray'+development_extension][player_stuff['DevelopmentPlanIndex'+development_extension]]
                                        apply_to_second -= apply_to_first
                                        player_stuff['DevelopmentPlanIndex'+development_extension] += 1
                                        if player_stuff['DevelopmentPlanIndex'+development_extension] == len(player_stuff['DevelopmentPlanCountedDaysArray'+development_extension]) :
                                            player_stuff['DevelopmentPlanCountedDaysArray'+development_extension] = []
                                            cont = True
                                    ...
                                    for name_of_stat in player_stuff['DevelopmentPlan'+development_extension] :
                                        try :
                                            player_stuff['Player Stats'][name_of_stat] += (apply_to_first*(player_stuff['DevelopmentPlan'+development_extension][name_of_stat][player_stuff['DevelopmentPlanIndex'+development_extension]-1]))
                                        except :
                                            pass
                                        player_stuff['Player Stats'][name_of_stat] += (apply_to_second*(player_stuff['DevelopmentPlan'+development_extension][name_of_stat][player_stuff['DevelopmentPlanIndex'+development_extension]]))
                                        for morale_changer in player_stuff['Morales'] :
                                            dev_morale_change = (1-((morale_changer/expected_morale)*(player_stuff['Bases'][name_of_stat]/harshest_day_projection)))
                                            if not morale_can_help :
                                                if dev_morale_change > 0 :
                                                    dev_morale_change = 0
                                            player_stuff['Player Stats'][name_of_stat] += dev_morale_change
                                    player_stuff['Morales'] = []
                                    
                                    
                                    positions_to_compute = ['CAM', 'CB', 'CDM', 'CF', 'CM', 'GK', 'LB', 'LM', 'LW', 'LWB', 'RB', 'RM', 'RW', 'RWB', 'ST']
                                    positions_to_compute.remove(player_stuff['Position'])
                                    real_position_rating, adds_percentages, next_to_one = calculate_player_rating(player_stuff['Player Stats'], player_stuff['Position'], player_stuff['Position'])
                                    player_stuff['Rating'] = str(real_position_rating)
                                    ratings_positions = {player_stuff['Position']:real_position_rating}
                                    for position_to_compute in positions_to_compute :
                                        ratings_positions[position_to_compute], adds_percentages2, next_to_one2 = calculate_player_rating(player_stuff['Player Stats'], position_to_compute, player_stuff['Position'], real_position_rating=real_position_rating)
                                    if team_progressing == team :
                                        #print('====================================================')
                                        already_p = False
                                        for key_changing in ratings_positions :
                                            if player_progressing in current_data[team_progressing+'_Lineup'] and ratings_positions[key_changing]-player_stuff['Positions'][key_changing] > 1 :
                                                print()
                                                if ratings_positions[key_changing] >= min([current_data[key_progressing][currently_contri]['Positions'][get_positions_from_formation(current_data[team_progressing+'_Formation'])[i_cc]] for i_cc, currently_contri in enumerate(current_data[team_progressing+'_Lineup'])]) :
                                                    print(player_progressing)
                                                    print(key_changing, ratings_positions[key_changing]-player_stuff['Positions'][key_changing])
                                        #print('====================================================')
                                    player_stuff['Positions'] = ratings_positions
                                    
                                    player_stuff['DaysUntilNextDevUpdate'] = 0
                                    done_count += 1
                    ...
                    if next_month :
                        ...
                        games_in_current_month, dates_of_games_in_month = update_game_month_stuff()
                    if current_data['WithoutYear'] in games_in_current_month :
                        sim_other_games(games_in_current_month)
                        standings_data_string, standingsinorder_data_string, standingsdata_data_string, topscorers_data_string, topscorersinorder_data_string, topassistors_data_string, topassistorsinorder_data_string, teamsinleague_data_string = get_league_specific_string(league_choice_index_basic)
                        teams_in_league = current_data[teamsinleague_data_string]
                    try :
                        current_data['NextOpponent'], current_data['NextStadium'], current_data['NextLeftOrRight'] = dates_of_games_in_month[current_data['WithoutYear']]
                        
                        current_data['StoppedOnMatchday'] = True
                    ...
                    for i, league_checking in enumerate(leagues_in_game) :
                        if current_data['WithoutYear'] == last_days_of_regular_season[league_checking] :
                            handle_end_of_regular_season(i)
                            games_in_current_month, dates_of_games_in_month = update_game_month_stuff()
                    if current_data['WithoutYear'] == day_of_season_reset :
