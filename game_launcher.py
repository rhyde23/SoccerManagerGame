...
import pygame, pickle, game, time
from os import listdir
from file_path_converter import convert_path
from schedule import get_schedule_prem, get_schedule_championship, get_schedule_league_one,get_schedule_league_two
...
teams_in_each_league = {
	...
	'English League One':['Hull City', 'Peterborough United', 'Sunderland', 'Portsmouth', 'Charlton Athletic', 'Ipswich Town', 'Oxford United', 'Blackpool', 'Fleetwood Town', 'Lincoln City', 'Doncaster Rovers', 'Burton Albion', 'Milton Keynes Dons', 'Shrewsbury Town', 'Crewe Alexandra', 'Accrington Stanley', 'Gillingham', 'Plymouth Argyle', 'Wigan Athletic', 'Bristol Rovers', 'AFC Wimbledon', 'Swindon Town', 'Northampton Town', 'Rochdale'],
	'English League Two':['Bolton Wanderers', 'Salford City', 'Cheltenham Town', 'Exeter City', 'Cambridge United', 'Morecambe', 'Leyton Orient', 'Tranmere Rovers', 'Oldham Athletic', 'Stevenage', 'Port Vale', 'Carlisle United', 'Forest Green Rovers', 'Bradford City', 'Scunthorpe United', 'Crawley Town', 'Colchester United', 'Newport County', 'Mansfield Town', 'Harrogate Town', 'Walsall', 'Barrow', 'Southend United', 'Grimsby Town'],
}
...
def get_league_stamina_average(league, current_data) :
	stamina_total = {esp_key:0 for esp_key in energy_same_positions}
	stamina_divider = {esp_key:0 for esp_key in energy_same_positions}
	for t_in_lea in current_data['TeamsInEachLeague'][league] :
		print(sorted([element for element in list(current_data.keys()) if element[-7:] == '_Lineup']))
		for p_in_tea_index, p_in_tea in enumerate(current_data[t_in_lea+'_Lineup']) :
			k_ea = find_energy_deduction_category(get_positions_from_formation(current_data[t_in_lea+'_Formation'])[p_in_tea_index])
			stamina_total[k_ea] += current_data[t_in_lea+'_Players'][p_in_tea]['Player Stats']['Stamina']
			stamina_divider[k_ea] += 1
	return {esp2_key:int(stamina_total[esp2_key]/stamina_divider[esp2_key]) for esp2_key in energy_same_positions}
		...
		for event in pygame.event.get() :
			if event.type == pygame.MOUSEBUTTONDOWN :
				if over_ts_next_page_button :
					...
					schedule_dicts = {'English Premier League':get_schedule_prem(), 'English Championship':get_schedule_championship(), 'English League One':get_schedule_league_one(), 'English League Two':get_schedule_league_two()}
					...
					current_data = {
						...
						'UnreadEmails':0,
						'StoppedOnMatchday':False,
						'NextStadium':'',
						'NextLeftOrRight':0,
						'NextOpponent':'',
						'WithoutYear':'August 31',
						'Arsenal_Players':{},
					...
					current_lineup_update = 1
					
					for l_index, le in enumerate(['English Premier League', 'English Championship', 'English League One', 'English League Two']) :
						...
						for t in t_i_l :
							...
							start = time.time()
							try :
								b_formation, b_lineup, b_team_rating = best_lineup({keyy:current_data[key][keyy]['Positions'] for keyy in current_data[key]})
								current_data[t+'_Formation'] = b_formation
								current_data[t+'_Lineup'] = new_formation_lefts_and_rights(get_positions_from_formation(b_formation), b_lineup, t)
								current_data[t+'_LineupUpdate'] = current_lineup_update
							except :
								print(t, 'didnt work')
								quit()
							print(time.time()-start)
							current_lineup_update += 1
							if current_lineup_update == 3 :
								current_lineup_update = 1
						
						
						
					#print({keyy:current_data['Manchester United_Players'][keyy]['Positions'] for keyy in current_data['Manchester United_Players']})
					...
					for siuuuu in current_data['TeamsInEachLeague'] :
						current_data[siuuuu+'_EDeductions'] = get_league_stamina_average(siuuuu, current_data)
						print(siuuuu, current_data[siuuuu+'_EDeductions'])
